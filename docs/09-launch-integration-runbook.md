...
| # | 交付物 | 本轮状态 | 证据位置 |
|---|---|---|---|
| 1 | 落地页部署为可访问 URL | **部分完成**：装置已在**局域网**可达并留痕（`http://192.168.31.35:8088/`）；**公网可达未完成** —— 零预算下无公网主机/域名，已按纪律提 board 裁定（见 §2.4） | §2 部署事实 · §3 冒烟回执 |
| 2 | 流量进入 | **未启动**（正确） | board 交互 `ba1deb68` 问题 1 已裁定 = **零预算、只用自然流量**；且无公网入口时无法引入流量 |
...
- 因此「落地页对外部署」的**公网部分**是资源决策，已按纪律提 issue 交互交 board：
  选项 = ① board/ops 提供公网主机 + 域名；② 批准零成本临时隧道（仅测试，地址会变）；③ 维持局域网可达、公网部署推迟到 T0。
...
3. 本部署**不构成**「对外投放已开始」：公网入口未定、流量未引入（§1 #2）。
...
| # | 事项 | 归属 | 状态 |
|---|---|---|---|
| 1 | 公网入口（主机 + 域名，或隧道授权） | board / ops | ⏳ 已提交互（§2.4） |
...
| 5 | 创作者链接与二维码按真实基址重生成（`links.csv` 现为 `landing.local` / `app.local` 占位） | Marketing Strategist | ⏳ 待 #1 |
| 6 | 商标与域名初筛（命名方向 A 的前置） | 超出本包授权 | ❌ 未做 |
| 7 | 本手册与两个新脚本**未登记进** `marketing/compliance/scan_manifest.json` 的 `device_files` | Marketing Strategist | ⏳ 待办：登记动作留给下一轮（避免在复审进行中改动扫描清单；本轮把清单留成 `hard_ban=18 / t0_gated=6 / 扫描目标=9` 不变，已实测） |
